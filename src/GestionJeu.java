...
public class GestionJeu {
    ...
    /**
     * Creez un jeu avec une hauteur et une largeur définie pour la zone du jeu, initialise une liste vide projectiles, d'objets, et d'aliens, initialise une liste de String
     * d'objets possible pour ensuite permettre de prendre un des types d'objets aléatoirement, initialise le score et créez les aliens pour le niveau 1.
     */
    public GestionJeu() {
        this.largeur = 100;
        this.hauteur = 60;
    ...
    /**
     * Vérifie si le vaisseau peut se déplacer vers la gauche (si la longueur de la chaine du vaisseau + sa position en x est supérieur à sa longueur en étant à posX de 0)
     * et le fait se déplacer vers la gauche si la condition est vérifié.
     * la vitesse du vaisseau augmente au fur et à mesure des niveaux passer (on pourrait répartir autrement dans le cas ou il y'a beaucoup de niveaux).
     */
    ...
    /**
     * Vérifie si le vaisseau peut se déplacer vers la droite (si la nouvelle position en X comprenenant la place que fait le vaisseau reste infèrieur à la bordure droite de la zone)
    ...
    /**
     *  chaque tir se doit d'être espacé l'un de l'autre avec un délai de 0.5s, ces tirs peuvent être différents en fonction d'un objet acquis par le vaisseau entèrieurement,
     * traiter par un switch case, on veut qu'un tir spéciale après l'acquisition d'un objet donc modeObjet est réinitialisé à défaut "Vanilla"
     */
    ...
    public EnsembleChaines getChaines() {
        ...
        ensemble.ajouteChaine(this.largeur - 20, this.hauteur - 3, "PV :" + vaisseau.vie); // affiche les pv restants avant de perdre la partie
        ensemble.ajouteChaine(this.largeur - 30, this.hauteur - 3, "NIVEAU :" + this.niveau); // affiche le niveau dans lequel on se situe
    ...
    /**
     * Controle le jeu à chaque tour, sa continuité, le comportement des élements (projectiles, alien, vaisseau, et les collision)
     */
    public void jouerUnTour() {
        if(vaisseau.vie == 0){  // dans le cas ou le vaisseau na plus de vie, on clear les listes pour ne plus afficher les élements du jeu 
            projectiles.clear();
            aliens.clear();
            return; 
        }
        ...
        // si un projectile rentre en collision avec un alien, l'alien touché est ajouté à une liste et la variable alienAtouche est affecté à true pour signaler au moins un alien toucher durant le tour
        for (Projectile proj : this.projectiles) {
            for (Aliens alien : this.aliens) {
                if (!(proj.estAlien()) && alien.contient((int) proj.positionX, (int) proj.positionY)) { // on s'assure que le projectile n'est pas issue d'un alien (au quel cas l'alien meurt en tirant)
        ...
        // si un alien à était touché durant le tour actuelle, il y'a une possibilité de 30% qu'un objet soit drop
        if (alienATouche) {
            double posYSpawnObjet = alienTouche.get(alienTouche.size() - 1).getPosYAlien(); // on s'assure que l'objet drop à la position du dernier alien touché 
        ...
        // gère les collisions entre les objets et le vaisseau, et dans le cas d'une collision attribut un statut aléatoire au vaisseau (l'aléatoire n'est pas calculée au moment ou l'instance d'un objet apparaît)
        ...
        // gère les collisions entre les projectiles tirées par des alien qui touche le vaisseau, décréemente de 1 la vie dans ce cas
        for (Projectile projectile : this.projectiles) {
        ...
        // actualise continuellement les projectiles et liste d'aliens légitime à rester sur le jeu (évite des calculs pour des projectiles qui ont touchées des aliens, évite conserver en mémoire des aliens touchées)
        this.aliens.removeAll(alienTouche);
        this.projectiles.removeAll(projectileQuiOntTouche);
        this.score.ajoute(alienTouche.size());


        // ajoute la fonctionnalité de tir aux aliens, avec une probabilité croissante en fonction du niveau et une part d'aléatoire
        for (Aliens alien : this.aliens) {
            alien.evolue();
        ...
        // fait avancer les projectiles déjà tirées et supprime les projectiles hors de la zone de jeu 
        ...
        // si la liste d'alien durant la manche (niveau) actuelle est de 0 , passe au niveau prochain niveau (possibilité de regarder en amont si un niveau+1 existe)
    ...
    /**
     * Ajoute une mécanique de niveau au jeu, permet setup l'environnement d'un niveau en réaction à la valeur de l'attribut niveau
     */
    public void niveauSuivant(){
        switch(niveau){
            ...
            this.aliens.addAll(Arrays.asList(
                ...
                new AlienEvoUn(this.largeur - 60, 120), // dépasse de la hauteur mais permet de la zone de jeu mais permet de faire des manches
                new AlienEvoUn(this.largeur - 60, 120),// on pourrait aussi bien combiner un compteur et s'assurer que tout les aliens sont mort pour faire
                new AlienEvoUn(this.largeur - 60, 120))); // pop des nouveaux aliens au sein d'un même niveau (implique de changer la condition qui incrémente le niveau dans GestionJeu)
            case 3:
